...
Request a large SCC machine to run this script. I would use 12 cores and a GPU or else the script may crash.

Local machine will kaboom
...
def log_likelihood(data, log_h, mu, log_sigma_eta, logit_phi):
    ...
    # Clamp log_h to avoid excessively large values in the loc parameter
    clamped_log_h = torch.exp(log_h).clamp(max=1e3)  # Adjust the max value as needed to prevent overflow
    trans_ll = dist.Normal(mu + phi * (clamped_log_h[:-1] - mu), sigma_eta).log_prob(clamped_log_h[1:]).sum()
...
# Extract the log volatility (h values)
log_h_samples = samples[:, 3:]
h_values = torch.exp(log_h_samples).mean(dim=0).numpy()

# Save h values to CSV
h_values_df = pd.DataFrame({'h_values': h_values})
h_values_df.to_csv('h_vals.csv', index=False)

print("h values saved to h_vals.csv")
